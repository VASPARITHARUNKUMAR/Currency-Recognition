...
def recognizedCurrency():
    ...
    if flag == 1:
        ...
        xi = float(input("Enter the currency rate for rupee"))
        xd = float(input("Enter the currency rate for dollor"))
        xe = float(input("Enter the currency rate for euro"))
        ...
        if currency == 'INR':
            text.insert(END, 'Currency in Euro : '+str(float(recognized) / xe)+"\n")
            text.insert(END, 'Currency in USD  : '+str(float(recognized) / xd)+"\n")
            cv2.putText(img_rgb, 'Currency in Euro : '+str(float(recognized) / xe), (10, 55),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
            cv2.putText(img_rgb, 'Currency in USD  : '+str(float(recognized) / xd), (10, 85),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
        if currency == 'USD':
            text.insert(END, 'Currency in Euro : '+str(float(recognized) /1.1)+"\n")
            text.insert(END, 'Currency in INR  : '+str(float(recognized) * xd)+"\n")
            cv2.putText(img_rgb, 'Currency in Euro : '+str(float(recognized) / 1.1), (10, 55),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
            cv2.putText(img_rgb, 'Currency in INR  : '+str(float(recognized) * xd), (10, 85),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2) 
        if currency == 'EURO':
            text.insert(END, 'Currency in USD : '+str(float(recognized) *1.1)+"\n")
            text.insert(END, 'Currency in INR  : '+str(float(recognized) * xe)+"\n")
            cv2.putText(img_rgb, 'Currency in USD : '+str(float(recognized) * 1.1), (10, 55),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
            cv2.putText(img_rgb, 'Currency in INR  : '+str(float(recognized) * xe), (10, 85),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)        
